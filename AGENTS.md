...
Production: <https://elearning.xid.my/>
...
- The root `index.html` is the brand catalog.
- Each brand lives in a top-level folder with its own `index.html` game catalog.
- Each game lives under its brand folder with its own `index.html` and local assets.
- Published brand and game folder paths are public contracts because Google Sites may link to them directly.
...
- Use relative asset and navigation URLs so pages work locally, on the custom domain, and on the GitHub Pages fallback URL.
- Treat path letter case as significant because GitHub Pages runs on a case-sensitive filesystem.
- Keep changes limited to the requested game, its brand catalog, and the root catalog only when adding or changing a brand.
